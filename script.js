...
function mode(){
  ...
  const apply=(isDark)=>{
    ...
    try{ localStorage.setItem('prefers.dark', isDark ? '1' : '0'); }catch{}
  };
  let saved=null;
  try{ saved=localStorage.getItem('prefers.dark'); }catch{}
  const prefersOS = (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches);
  const startDark = (saved!==null) ? (saved==='1') : prefersOS;
  apply(startDark);
